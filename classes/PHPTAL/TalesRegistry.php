...
class PHPTAL_TalesRegistry {
	...
	public function registerPrefix($prefix, $callback) {
		if($this->isRegistered($prefix)) {
			throw new Exception(sprintf('Expression modifier "%s" is already registered.',$prefix));
		}
		...
		if(is_array($callback)) {
			...
			if(!$class->isSubclassOf('PHPTAL_Tales')) {
				throw new Exception('The class you want to register does not implement "PHPTAL_Tales".');
			}

			$method = new ReflectionMethod($callback[0], $callback[1]);

			if(!$method->isStatic()) {
				throw new Exception('The method you want to register is not static.');
			}
			...
			if(!function_exists($callback)) {
				throw new Exception('The function you are trying to register does not exist.');
			}
		}
	...
	public function getCallback($prefix) {
		if(!$this->isRegistered($prefix)) {
			throw new Exception(sprintf('Expression modifier "%s" is not registered.', $prefix));
		}
